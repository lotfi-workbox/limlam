...
import com.saeedlotfi.limlam.userInterface.viewModels.FoldersViewModel
import javax.inject.Inject

open class FoldersFragment : BaseFragment(), IOnBackPressed, FilterAdapter {
    ...
    override fun onCreateView(
        ...
        ViewModelProvider(viewModelStore, viewModelFactory).also { vmp ->
            viewModel = vmp[FoldersViewModel::class.java]
        }
        configRecyclerView()
    ...
    override fun onStart() {
        LocalBroadcastManager.getInstance(activity as MainActivity).apply {
            registerReceiver(updateUi, IntentFilter(Constants.BC_ACTION_REFRESH_FOLDERS))
        }
        super.onStart()
    }
    ...
    override fun onDestroyView() {
        viewModel.listOfFolders.removeObservers(viewLifecycleOwner)
    ...
    private fun configRecyclerView() {
        viewModel.listOfFolders.observe(viewLifecycleOwner) {
            it?.also { foldersList ->
                foldersList.sortBy { fm -> fm.name }
                userInterface.view.rvList.adapter = null
                adapter.init(
                    inputList = foldersList as MutableList<com.saeedlotfi.limlam.domain.model.AbstractDoModel>,
                ...
                if (foldersList.isEmpty() && userInterface.view.flProgressBar.visibility != View.VISIBLE) {
                    userInterface.view.tvEmpty.visibility = View.VISIBLE
                } else if (foldersList.isNotEmpty()) {
                ...
                adapter.submitList(foldersList as? List<com.saeedlotfi.limlam.domain.model.AbstractDoModel>?)
            }
